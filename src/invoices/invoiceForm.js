...
class Form extends Component {
    ...
    addLaborItem = () => {
        ...
        if (this.state.addLaborDescription === '') {alert('Labor description cannot be blank'); return;}
        if (this.state.addLaborCost === '') {alert('Labor cost cannot be blank'); return;}
    ...
    addMaterialItem = () => {
        ...
        if (this.state.addMaterialDescription === '') {alert('Material description cannot be blank'); return;}
        if (this.state.addMaterialUnitCost === '') {alert('Material unit cost cannot be blank'); return;}
        if (this.state.addMaterialQuantity === '') {alert('Material quantity cannot be blank'); return;}
    ...
    saveInvoice = async (viewWhenDone) => {
        ...
        if (!!this.state.invoiceId) { // Update existing
            ...
            console.log('New called', data);
                try {
                    const createdInvoice = await Api.axios.post(`/invoice`, data.invoice);
                    console.log('createdInvoice', createdInvoice);
                    if (data.laborItems.length) {
                        // Add invoice id's here
                        const createdLaborItems = await Api.axios.post(`/invoice/laborItems`, data.laborItems);
                        console.log('createdLaborItems', createdLaborItems);
                    }
                    if (data.materialItems.length) {
                        const createdMaterialItems = await Api.axios.post(`/invoice/materialItems`, data.materialItems);
    ...
    render() {
        return (
            <form style={{margin: '0 20px 0 0'}} autoComplete="off" noValidate>
                ...
                <div>
                    <button type="button" onClick={() => {this.saveInvoice(false)}}>Save</button>
                    <button type="button" onClick={() => {this.saveInvoice(true)}}>Save and View invoice</button>
                </div>

                Add Labor Description<br/>
                <input type="text" value={this.state.addLaborDescription} onChange={e => this.setState({addLaborDescription: e.target.value})}/><br/>
                Add Labor Cost<br/>
                <input type="number" value={this.state.addLaborCost} onChange={e => this.setState({addLaborCost: e.target.value})}/>
                <div><button onClick={this.addLaborItem} type="button">Add this labor item</button></div>

                Add Material Description<br/>
                <input type="text" value={this.state.addMaterialDescription} onChange={e => this.setState({addMaterialDescription: e.target.value})}/><br/>
                Add Unit Cost<br/>
                <input type="number" value={this.state.addMaterialUnitCost} onChange={e => this.setState({addMaterialUnitCost: e.target.value})}/><br/>
                 Add Material Quantity<br/>
                <input type="number" value={this.state.addMaterialQuantity} onChange={e => this.setState({addMaterialQuantity: e.target.value})}/>
                <div><button onClick={this.addMaterialItem} type="button">Add this material item</button></div>
                <h2>Labor Items</h2>
                {
                    this.state.laborItems.map(l => {
                        return <div key={l.id}>{l.description} {currencyFormatter(l.cost)}<button type="button">Edit</button> <button type="button">Delete</button></div>
                    })
                }
                <h2>Material Items</h2>
                {
                    this.state.materialItems.map(m => {
                        return <div key={m.id}>{m.description} {currencyFormatter(m.unitCost)} {m.quantity} total:{currencyFormatter(m.quantity*m.unitCost)}<button type="button">Edit</button> <button type="button">Delete</button></div>
                    })
                }
            </form>
        )
